...
\addcontentsline{toc}{chapter}{Zitieren, Abbildungen, Quelltext}
...
\begin{itemize}
  ...
  \item Verkürzungen in Zitaten werden mit [...] gekennzeichnet.
\end{itemize}
...
\textbf{Beispiel 1: Längeres Zitat mit Verkürzung und anschließender Einordnung}

\begin{quotation}
\noindent
\emph{\glqq We tend to think of navigating a website as clicking from page-to-page via some kind of global navigation [...] When it comes to a single page, we often think scrolling is the one and only way to move from one end to the next.\grqq}~\cite{bib:bradley1}
\end{quotation}
...
\textbf{Kurzregeln zu Footnotes vs. Zitation:}

\begin{itemize}
  \item Zitate verwenden, wenn eine Aussage, Zahl oder Idee aus einer
  Quelle stammt und nachprüfbar belegt werden muss.
  \item Fußnoten \footnote{Kurze Nebeninformationen werden in Footnotes gegeben} verwenden, wenn eine kurze Nebeninformation, ein
  Begriffshinweis oder eine ergänzende Erklärung den Lesefluss sonst stören
  würde.
\end{itemize}

\textbf{Beispiel 2: Begriffserklärung als Fußnote}

In diesem Abschnitt wird der
Begriff \emph{NPR} verwendet.\footnote{NPR steht für \emph{Non-photorealistic
Rendering} und beschreibt Techniken, die bewusst nicht fotorealistisch wirken.}

\textbf{Beispiel 3: Methodischer Hinweis als Fußnote}

Die Messwerte wurden in einer
Testumgebung erhoben.\footnote{Die Tests liefen auf einem isolierten System ohne
...
\textbf{Beispiel: Absatz mit KI-Unterstützung}
